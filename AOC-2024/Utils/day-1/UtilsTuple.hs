...
class FromListToTuple (n :: Nat) a t | n a -> t where
  fromListToTuple :: [a] -> t
...
fromListToTupleInstance :: Int -> TH.DecsQ
fromListToTupleInstance n = do
  let nType = TH.LitT (TH.NumTyLit (fromIntegral n))
  typeName <- TH.newName "a"

  let classType =
        TH.ConT (TH.mkName "FromListToTuple")
          `TH.AppT` nType
