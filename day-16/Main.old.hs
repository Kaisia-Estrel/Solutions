...
import Data.List (find, findIndices, foldr, nub, sortOn, groupBy, nubBy)
...
filterValid2 point symbols =
  -- nubBy (\x y -> fst y == fst x) $ 
  nubBy(\x y ->  fst y == fst x) . filter (\x -> isJust $ find (== x) symbols) $ genValid point
...
main = do
  ...
  let b = map readToInt
          ...
          . constructNumbers parsed
          . nub
          . map (`getAdjacent` nums) . concat . filter (\x -> length x == 2) $ filterByGear valid2 syms2
  ...
  -- print $ filterValid2 (1,3) [(1,2),(1,4)]
  -- print . filter (\x -> length x == 2) $ filterByGear valid2 syms2

-- print . nub $ map (`getAdjacent` nums) valid
